  ...
  return (
    <div className={`px-8 py-10 rounded-3xl flex flex-col ${props.classNames}`}>
      <div className="flex justify-between items-start">
        <span className="text-[3rem] font-bold tracking-[-0.75px] text-headings">{props.title}</span>
        <img src={`./icons/${props.symbol}-circle.svg`}></img>
      </div>
      <span className="text-[2.4rem] font-semibold tracking-[-0.6px] text-headings">{props.amount}</span>
